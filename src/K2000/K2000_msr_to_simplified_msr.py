...
def index_GFA_edges(MSR,unitigs,k):
    ...
    for msr in MSR.traverse():
        ...
        # out_edges[x_id]=get_right_edges(MSR,msr,x_id,unitigs,k)
        # if out_edges[x_id]==None: continue
...
def remove_this_similar_msr(MSR,msr,out_edges):
    ...
    for target in out_edges[x_id]:                                                  # we could compare x_id to ALL other msr. However its compared only to those that can be accessible, ie those that are son on soons (without taking the direction into account).
